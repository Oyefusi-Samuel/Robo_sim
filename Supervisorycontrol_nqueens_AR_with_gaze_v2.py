...
smooth_gaze_vector = gaze_tracking.GazeSmoothTracking(alpha=0.3)

screen_x = 0
screen_y = 0

# Use this variable to access the gaze coordinates for interface logic
screen_coordinates = (screen_x, screen_y)

def frame_to_gaze_screen_coordinates(frame):
    """Given the webcam frame, print the gaze screen coordinates onto the cv2 named window.
    Also return the gaze screen coordinates for interface logic"""
    # update the screen coordinates for the rest of the program logic to use
    ...
    if eye_landmarks.multi_face_landmarks:    
        gaze_estimate_x, gaze_estimate_y, gaze_world = gaze_tracking.gaze_to_screen_point(eye_landmarks.multi_face_landmarks[0].landmark,
                                                                            width,
                                                                            height,
                                                                            screen_width,
                                                                            screen_height)
...
# ═══════════════════════════════════════════════════════
#  Multi-threading (one captures webcam frames and one processes on a copy of the frame)
# ═══════════════════════════════════════════════════════
class WebcamInput:
    def __init__(self):
...
MINI_ORIG=(800,130); MINI_SQ=26*10; MINI_N=8; GRID_SIZE = 10
...
class ChessBoard:
    ...
    def get_overlay(self, frame, placed_map, queen_row, src_col, dst_col):
        if self.cached_board is None:
            # initialize the cache
            height, width = frame.shape[:2]
            board = np.zeros((height, width, 3), dtype=np.uint8)
            draw_mini_board(board, placed_map, queen_row, src_col, dst_col)
            self.cached_board = board
            self.last_board_state = (placed_map, queen_row, src_col, dst_col)
        
        if self.last_board_state != (placed_map, queen_row, src_col, dst_col):
            # only redraw if the board changes
            height, width = frame.shape[:2]
            board = np.zeros((height, width, 3), dtype=np.uint8)
            draw_mini_board(board, placed_map, queen_row, src_col, dst_col)
            self.last_board_state = (placed_map, queen_row, src_col, dst_col)
            self.cached_board = board
            mask = cv2.cvtColor(board, cv2.COLOR_BGR2GRAY) > 0
            frame[mask] = board[mask]
        
        else:
            # draw cached board
            mask = cv2.cvtColor(self.cached_board, cv2.COLOR_BGR2GRAY) > 0
            frame[mask] = self.cached_board[mask]

chess_board = ChessBoard()

def draw_ar_overlay(frame, move_idx, total, queen_row, src_col, dst_col,
                    gesture, landmarks, status_txt, placed_map, ft, chess_board):
    
    draw_top_banner(frame, move_idx, total, queen_row, src_col, dst_col, gesture)
    chess_board.get_overlay(frame, placed_map, queen_row, src_col, dst_col)
...
cv2_named_window = "Supervisory Control — N-Queens"
cv2.namedWindow(cv2_named_window, cv2.WINDOW_NORMAL)
cv2.setWindowProperty(cv2_named_window, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
cv2.resizeWindow(cv2_named_window, 3840, 2400)
if not cap.isOpened():
...
try:
    ...
    while move_queue:
        ...
        while decision is None:
            ...
            if ret:
                ...
                (screen_x, screen_y) = frame_to_gaze_screen_coordinates(frame)
    ...
    if use_multithreading:
        ...
        cap.release()
    p.disconnect(); cv2.destroyAllWindows()