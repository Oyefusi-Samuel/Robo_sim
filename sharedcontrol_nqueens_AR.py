...
def do_reset():
    global placed, cur_row, cursor, status
    clear_all_hl(); clear_dots(); hide_ghost(ghost)
    for i,a in enumerate(HOME): p.resetJointState(robot,i,a)
    set_fingers(robot, FINGER_OPEN*0.5)
    spawn_queens()
    placed={}; cur_row=0; cursor=0
...
def do_undo():
    ...
    p.resetBaseVelocity(queens[row],[0,0,0],[0,0,0])
    q_pos[row]=sp; cur_row=row; cursor=0
...
try:
    while p.isConnected():
        ...
        update_highlights(cur_row, safe, placed)
        move_ghost(ghost, sq_world(cur_row, sel))

        # HUD
        status = (f"Col {sel} RISKY (leads to dead end)" if warn
                  else f"Row {cur_row} → Col {sel}  |  ENTER to place")
        draw_hud(cur_row, len(placed), status)

        # Terminal
        sys.stdout.write(
            f"\r  ROW {cur_row}  sel=C{sel}  "
        ...
        if confirm:
            hide_ghost(ghost)
            clear_row(cur_row)
            flash_cell(cur_row, sel)

            src_pos = q_pos[cur_row]
            dst_pos = sq_world(cur_row, sel)
            print(f"\n\n  ► Row {cur_row} → Col {sel}")

            cid = pick(robot, queens[cur_row], src_pos)
            place(robot, queens[cur_row], cid, dst_pos)

            q_pos[cur_row] = dst_pos
            placed[cur_row] = sel
            add_dot(cur_row, sel)
            cur_row += 1; cursor = 0
