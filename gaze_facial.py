...
KB_OVERRIDE_SECS  = 1.0   # keyboard takes priority over gaze for this long
...
_kb_last_press = [0.0]    # timestamp of last keyboard cursor key — gaze yields
...
def gaze_worker(cap, cal_data):
    ...
    while not _stop_gaze.is_set():
        ...
        # compute_expression does frame=cv2.flip() internally, so putText
        # writes to a LOCAL copy — we must capture the return value.
        # We also pass an un-flipped copy so the internal flip is correct.
        try:
            annotated = compute_expression(cv2.flip(frame, 1))
            if annotated is not None:
                frame = cv2.flip(annotated, 1)  # flip back to match gaze overlay
        except Exception:
            pass   # never let expression tracking crash the gaze thread
...
try:
    while p.isConnected():
        ...
        if not CAL_FALLBACK:
            # Only let gaze move cursor when keyboard hasn't been used recently
